###################################################################################################
### This Code was written to extract and plot residues of 2 nucleon separation energy from      ###
...
#                print (mtN[i],Z,N,S2n[(N,Z,i)],S2p[(N,Z,i)])
 ...
 beta3_cut = 0.01; beta3_cap = beta2_cap = 1000
 oct_nuc = {}; oct_nuc1 = {}
    ...
    for line in l[i]:
        ...
        try:
            ...
            if isNum(data1) and isNum(data2) :
                ...
                if abs(float(data2)) >= beta3_cut and abs(float(data2)) <= beta3_cap and abs(float(data1)) <= beta2_cap:
                    oct_nuc[(N,Z,i)] = True
                else:
                    oct_nuc[(N,Z,i)] = False
...
#  S2n[(102,64,12)] = S2n[(102,64,13)]
 ...
 for i in it.chain(range(1,7), range(14,17), range(23,30)):
  #res2_p[mtN[i]] = 0
  # Only AME2016 has binding energy in this code, for now let's just focus on AME2016 residues
...
#        print (Z,N)
 ...
 ### l= 1: residue sequence for y-axis plotting                                                  ###
 ...
 #(proton #; theory masstable index; experiment dat index; data type 0: S2n, 1: S2p, 2: S1n; 0: neutron array, 1: residue array)
 ResC = {}
 ...
 ###################################################################################################
    ...
    if ( k==0 or k == 5 or k == 4 and (pSave == 1 or pSave == 3 or pSave == 6) ):
     ...
     if k == 0:
        fN = fN + "S2n/Residual_s2n_" + mtN[i] + "_" + mtN[j] + "." + fFormat
     if k == 5:
        fN = fN + "S2n/S2n_" + mtN[i] + "." + fFormat
     if k == 4:
        fN = fN + "BE/BE_" + mtN[i] + "." + fFormat
     count = 0      #counts how many nuclei data point is in the plot
     for Z in range(zS,zE+1,2):
      if Z%4: line_type = 'r-'; line_width=0.2
      else: line_type = 'b-'; line_width=0.2
      if k == 4: line_marker = '.'; line_marker_size = 2
      else: line_marker = None; line_marker_size = None
      # Use the below statement to single out an isotopic chain you're interested in
#      if Z == 6 and i == 23: line_type = 'g-'; line_width=0.5
      plt.plot(np.array(ResC[(Z,i,j,k,0)]), np.array(ResC[Z,i,j,k,1]),line_type,lw=line_width,marker = line_marker,
     ...
     if (i == 11 and j == 12):
      ...
      if k == 0:
          ...
          plt.text(125,14,"rms = " + str(round( s2n_rms[(i,j,k)],6 )) )
 ...
 ###################################################################################################
    ...
    if ( k==1 or k == 6 and (pSave == 2 or pSave == 3 or pSave == 6) ):
     ...
     if k == 1:
        fN = fN + "S2p/" + mtN[i] + "_" + mtN[j] + "_S2p_residue" + "." + fFormat
     if k == 6:
