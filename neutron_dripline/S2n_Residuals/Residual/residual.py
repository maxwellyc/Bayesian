...
def residues(saveNum, odevity, tFormat):
 ...
 #!! 11-AME2003, 12-AME2016, 17-TRIUMF, 18-RIKEN                                                 !!#
 ...
 mtN = ["","SkM*","SkP","SLy4","SV-min","UNEDF0","UNEDF1","DD-ME2","DD-ME$\delta$","DDPC1","NL3*",
        "AME2003","AME2016","JYFLTRAP2017","FRDM2012","HFB24","UNEDF2","TRIUMF2018","RIKEN2018"]
 ###################################################################################################
 ###                                      CAUTION! WARNING!                                      ###
 ###                                      CAUTION! WARNING!                                      ###
 ###                                      CAUTION! WARNING!                                      ###
 ...
 f21 = open("data/2016AME_ME.dat")
 aAME0 = [1,0,5,6]
 
 #Additional experimental data after 2016:
 # JYFLTRAP 2017 rare earth see email communications w/ Witek
 # https://journals.aps.org/prl/pdf/10.1103/PhysRevLett.120.062503 , TRIUMF 2018 titanium
 # https://journals.aps.org/prl/pdf/10.1103/PhysRevLett.121.022506 , RIKEN 2018 calcium
 ...
 # Mass Excess from 2018 TRIUMF / RIKEN, AME2016
 l[19] = f19.readlines(); l[20] = f20.readlines(); l[21] = f21.readlines()
 ...
 #!! Masstable index:                                                                            !!#
 #!! 1-SKMS, 2-SKP, 3-SLY4, 4-SVMIN, 5-UNEDF0, 6-UNEDF1, 7-DDME2, 8-DDMED, 9-DDPC1, 10-NL3S,     !!#
 #!! 11-AME2003, 12-AME2016, 13-JYFL2017, 14-FRDM2012, 15-HFB24, 16-UNEDF2, 17-TRIUMF, 18-RIKEN  !!#
 #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!#
    ...
    try:
     ...
     if (isNum(data)):
      ...
      if (isNum(data)):
       ...
       continue
 #2018 data (TRIUMF, RIKEN data), use a2018
...
# S1n[(37,20,17)] = 1.931318 ; S1nErr[(37,20,17)] = 1.021078
 ...
 #!! 11-AME2003, 12-AME2016, 13-JYFL2017, 17-TRIUMF, 18-RIKEN  !!#
 # If 13, 17, 18 has data points, do a search in its vicinity, any S1/2n, S1/2p generated using it should be stored in 13, 17 or 18.
 for i in it.chain([13,17,18]):
  for Z in range(2,zMax+1):
   for N in range(2,nMax+1):
    if (N,Z,i) in BE:
...
#    for N in range(2,nMax+1):
#     if (N,Z,i) in S1p:
#      print (f"{Z}\t{N}\t{i}\t:{S1p[(N,Z,i)]}\t{S1pErr[(N,Z,i)]}")
...
#  print (rms_s2n[(i,11)])
 ...
 f16.close(); f17.close(); f18.close(); f19.close(); f20.close(); f21.close()
...
#     plt.close()
 ...
 #S1n
 if ( saveNum == 1):
  outputLabel = "Z," + "N," +  "AME2003," +  "AME2003_sd," +  "AME2016," +  "AME2016_sd," +  "JYFLTRAP2017," +  "JYFLTRAP2017_sd,"  +  "TRIUMF2018," +\
  "TRIUMF2018_sd," + "RIKEN2018," + "RIKEN2018_sd," + "SKM*," +  "SKP," +  "SLY4," +  "SVMIN," +  "UNEDF0," +  "UNEDF1," +  "UNEDF2," +\
   "FRDM2012," +  "HFB24"
  outputStr = ""
  #count0, count1:scalars, count how many theories have residue for a specific nuclei
  ...
  for Z in range(zS,zMax1,zOdd):
   for N in range(nS,nMax1,nOdd):
    outputStr = "\n" + str(Z) + "," + str(N) + ","
    count = 0
    # experimental data, 11-AME2003, 12-AME2016, 13-JYFLTRAP2017, 17-TRIUMF2018, 18-RIKEN2018
    for i in it.chain(range(11,14), [17,18]):
     if ( (N,Z,i) in S1n ):
 ...
 ###################################################################################################
 # Save type switch: 0: No save; 1: S1n, 2: S2n, 3: S1p, 4: S2p, 5: residual
