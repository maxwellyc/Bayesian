...
def residues():
 ...
 #!! 11-AME2003, 12-AME2016, 17-TRIUMF                                                           !!#
 ...
 f1 = open("data/SKMS_all_nuclei-no_drip_lines.dat"); f2 = open("data/SKP_all_nuclei-no_drip_lines.dat")
 f3 = open("data/SLY4_all_nuclei-no_drip_lines.dat"); f4 = open("data/SV-MIN_all_nuclei-no_drip_lines.dat")
 f5 = open("data/UNEDF0_all_nuclei-no_drip_lines.dat"); f6 = open("data/UNEDF1_all_nuclei-no_drip_lines.dat")
 f16 = open("data/UNEDF2_all_nuclei-no_drip_lines.dat")
 #column indices of Z, N, S2n, S2p, S1n. ME stands for MassExplorer
 aME = [1,2,8,6,7]
 #
 ...
 aFRDM = [0,1,13]
 #colum indices of Z, A, Mass excess, S1n for HFB-24
 aHFB24 = [0,1,9,6]
 ...
 #column indices of Z, N, S2n/S1n, S2n/S1n errors, S2p, S2p errors (error data only exists for AME)
 #AME stands for Atomic Mass Evaluation, this aAME is good for both *_S2 file and *_S1 file
 aAME = [0,1,2,3]
 #JYFLTRAP 2017 data
 f13 = open("data/rare_earthS2n.dat")
 #column indices of Z, N, S2n, S2n errors
 aJYFL = [0,1,2,3,4,5]
 ...
 #!! Masstable index:                                                                            !!#
 #!! 1-SKMS, 2-SKP, 3-SLY4, 4-SVMIN, 5-UNEDF0, 6-UNEDF1, 7-DDME2, 8-DDMED, 9-DDPC1, 10-NL3S,     !!#
 #!! 11-AME2003, 12-AME2016, 13-JYFLTRAP2017, 14-FRDM2012, 15-HFB24, 16-UNEDF2, 17-TRIUMF        !!#
 #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!#
 ...
 # Manually adding 55,56,57 calcium masses calculated S1/2n data points
 ...
 ###################################################################################################
 # As of now, HFB-24 and FDRM2012 only contain mass excess or binding energy inputs,
 # Converting to separation energy below:
 #FRDM-2012 & HFB-24
 d_cache = 0.0
  ...
  for Z in range(2,zMax+1):
   for N in range(2,nMax+1):
    ...
    if ( ( (N,Z,i) in BE) and ( (N,Z-2,i) in BE) ):
     ...
     #if d_cache >= 0.0 :
     S2p[(N,Z,i)] = d_cache
 #Only FRDM-2012 needs to calculate S1n from Binding
 for i in range(14,15):
  for Z in range(2,zMax+1):
   for N in range(2,nMax+1):
 ...
 if ( saveNum == 2 or saveNum == 3):
  ...
  for Z in range(zS,zMax1,zOdd):
   for N in range(nS,nMax1,nOdd):
    ...
    outputStr = "\n" + str(Z) + "," + str(N) + ","
    count = 0;
    # experimental data, 11-AME2003, 12-AME2016, 13-JYFLTRAP2017, 17-TRIUMF 2018
